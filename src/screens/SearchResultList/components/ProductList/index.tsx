...
import { SearchProductList, SearchType, SortTypes, getSearchProductList } from "axios/api/search";
...
const ProductList: React.FC<ProductListProps> = ({ searchContent }) => {
    const [list, setList] = useState<SearchProductList>([])
    ...
    const sortTabOpts = [
        ...
        {
            title: "销量",
            value: SortTypes.Sold,
        },
        {
            title: "价格升序",
            value: SortTypes.AscendingPrice,
        },
        {
            title: "价格降序",
            value: SortTypes.DescendingPrice,
        }
    ]
    const handleGetMoreList = async () => {
        ...
        setPageNo(newPageNo)
        setLoadingMore(true);
        const res = await getSearchProductList({ searchContent, sortType, pageNo: newPageNo });
    ...
    const handleinitList = async () => {
        setRefreshing(true)
        const res = await getSearchProductList({ searchContent, sortType, pageNo: pageNo });
        setRefreshing(false)
        setList(res.data);
    }
